    ...
    arr
        .then((allPlaces) => {
            ...
            let index = 0,
                maxScore = -10000;
            for (r in results) {
                if (maxScore < results[r]) {
                    maxScore = results[r];
                    index = r;

                }
            }
            console.log("choosing the max for safest route", maxScore, index);

            res.render('map', { src: req.query.src, dest: req.query.dest, index: index, API_KEY: API_KEY_GLOBAL });
        })
        .catch((error) => {
            console.log(error);
